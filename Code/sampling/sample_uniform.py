...
from Methods.utils.priors import Uniform_prior, vectorize_matrix, export_samples
...
for frequency in frequencies:
    ...
    upper = vectorize_matrix(upper[:,:1+K*lags[frequency]]).reshape([K*(1+K*lags[frequency]),1]).astype(float)
    lower = vectorize_matrix(lower[:,:1+K*lags[frequency]]).reshape([K*(1+K*lags[frequency]),1]).astype(float)

    fixed_lower = -1 /(components[frequency]*lags[frequency])
    fixed_upper = 1  /(components[frequency]*lags[frequency])

    # if frequency=="Monthly cicle" or factors_from=="PCA":
    #     fixed_lower = -1 /(components[frequency])
    #     fixed_upper = 1  /(components[frequency])
    # else:
    #     fixed_lower = -1
    #     fixed_upper = 1

    if factors_from=="PCA":
        upper = np.ones_like(upper)*fixed_upper
        lower = np.ones_like(lower)*fixed_lower
    else:
        upper = np.nan_to_num(upper, nan=fixed_upper)
        lower = np.nan_to_num(lower, nan=fixed_lower)

    intervals[frequency+str(lags[frequency])] = np.concatenate([lower, upper], axis=1)
priors ={}


for frequency in frequencies:
    prior = Uniform_prior(interval = intervals[frequency+str(lags[frequency])])

    priors[frequency+str(lags[frequency])] = prior
...
for starting_date in starting_dates:
    ...
    for frequency in frequencies:
        ...
        folder_name = f"F_{frequency}_L_{lags[frequency]}_f_{factors_from}_w_{number_of_paths}_d_{starting_date}"

        data = my_model.decomposed_dict[factors_from][frequency][key_word_dict[factors_from]][:components[frequency]]

        priors[frequency+str(lags[frequency])].aggregate_likelihood(data, number_of_lags= lags[frequency], jumps= jumps[frequency])

        sample_dict = priors[frequency+str(lags[frequency])].sample(number_of_paths)

        print(f"Done for {frequency} and {lags[frequency]}")
