...
factors_from = "FSA"
estimation_method= "YW"
number_of_lags=[1,2,3,4,5]
...
test_data = pd.DataFrame(columns=["Model", "Frequency", "Lags", "Factors"])
...
for number_of_lags_i in number_of_lags:
    ...
    my_model.build_VAR(estimator = estimation_method, components=components, jumps=jumps, lags=lags, components_from = factors_from)

    for frequency in frequencies:
        this_freq_dict=my_model.processes[frequency].get_information_criterias()
        this_freq_dict_2=my_model.processes[frequency].get_error_tests()

        this_freq_dict = this_freq_dict | this_freq_dict_2


        this_freq_dict["Model"]=f"{frequency}_ {number_of_lags_i} lags _{factors_from}"
...
test_data.to_excel(output_directory+f"\\VAR_{estimation_method}_ {number_of_lags} lags _{factors_from}.xlsx")
